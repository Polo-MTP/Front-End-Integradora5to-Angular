...
export class SensorCard {
  ...
  getTemperatureStatus(value: number): string {
    if (value >= 23 && value <= 26) return 'Óptimo';
  ...
  getHumidityStatus(value: number): string {
    ...
    if ((value >= 50 && value < 60) || (value > 70 && value <= 80))
      return 'Aceptable';
  ...
  // Métodos para Turbidez (tbz)
  getTurbidityStatus(value: number): string {
    if (value < 500) return 'Excelente';
    if (value < 700) return 'Aceptable';
    return 'Turbia';
  }

  getTurbidityPercentage(value: number): number {
    const max = 1041; // Máximo esperado de tu sensor
    return Math.min(100, (value / max) * 100);
  }

  getTurbidityBarClass(value: number): string {
    if (value < 500) return 'bg-green-100';
    if (value < 700) return 'bg-yellow-100';
    return 'bg-red-100';
  }

  // Métodos para TDS (tds)
  getTDSStatus(value: number): string {
    if (value <= 300) return 'Excelente';
    if (value <= 500) return 'Aceptable';
    return 'Alto';
  }

  getTDSPercentage(value: number): number {
    const max = 1000; // Máximo en la escala visual
    return Math.min(100, (value / max) * 100);
  }

  getTDSBarClass(value: number): string {
    if (value <= 300) return 'bg-green-100';
    if (value <= 500) return 'bg-yellow-100';
    return 'bg-red-100';
  }

  // Métodos para Nivel de Agua (niv)
  getWaterLevelStatus(value: number): string {
